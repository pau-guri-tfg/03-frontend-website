...
export default function PlayerCard({ player }: { player: GamePlayer }) {
  ...
  return (
    <a href={`/summoner/${player.riotIdGameName}/${player.riotIdTagLine}`} className='flex items-center justify-between gap-3 px-6 py-3 cursor-pointer rounded-3xl bg-dark-blue group'>
      <div className='flex items-center w-1/4 gap-3'>
        <div className={'relative shrink-0 w-10 h-10 border' + (player.team === "ORDER" ? " border-riot-blue" : " border-riot-red")}>
          ...
          <div className='absolute bottom-0 right-0 flex items-center justify-center w-4 h-4 translate-x-1/2 translate-y-1/2 rounded-full bg-dark-blue'>
            <span className='pt-1 font-serif text-xs font-bold'>{player.level}</span>
          </div>
        </div>
      ...
      <div className='flex items-center gap-3 shrink-0'>
        <div className='flex flex-col items-center gap-1'>
          ...
          <span className='font-serif text-lg font-bold leading-none'>{player.scores.creepScore}</span>
        </div>
        <div className='flex flex-col items-center gap-1'>
          <span className='text-sm text-white/40'>VS</span>
          <span className='font-serif text-lg font-bold leading-none'>{player.scores.wardScore}</span>
        </div>
      </div>
