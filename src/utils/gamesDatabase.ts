...
export function fetchGamesByChampionEndpoint(championName: string, endpoint: GameDatabaseEndpoint, limit?: number, offset?: number) {
  return axios.get(`${import.meta.env.VITE_BACKEND_URL}/games-by-champion/${championName}/${endpoint}?api_key=${import.meta.env.VITE_BACKEND_API_KEY}` + (limit ? `&limit=${limit}` : "") + (offset ? `&offset=${offset}` : ""));
}